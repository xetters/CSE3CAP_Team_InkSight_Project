...
function generateHTML(title, wordData, sentData) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
function initDownloads(downloadBtn) {
  downloadBtn.addEventListener('click', () => {
    const html = generateHTML('InkSight Analysis Report', lastResult, sentimentData);
...
function downloadSection(section) {
  if (section === 'word-analysis') {
    const html = generateHTML('Word Analysis', lastResult, null);
    downloadHTML('inksight-word-analysis.html', html);
  } else if (section === 'sentiment') {
    const html = generateHTML('Sentiment Analysis', null, sentimentData);
    downloadHTML('inksight-sentiment.html', html);
  } else if (section === 'keyness-stats') {
    alert('Keyness Statistics download will be available soon.');
  }
}
